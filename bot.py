...
SUPPORT_USER_ID = (
    ...
    else 502791142  # твой дефолтный ID
)
...
def build_sarcastic_prompt(user_text: str) -> str:
    ...
    return (
        ...
        f"{user_text}\n\n"
        "Теперь придумай одно подходящее саркастическое сообщение в чат, обращаясь к Максиму. Только текст сообщения, без пояснений."
    )
...
def build_supportive_prompt(user_text: str) -> str:
    ...
    return (
        ...
        "Твоя задача — написать одно короткое сообщение по-русски, которое поддерживает и усиливает Максима: "
        "похвали его, вырази уверенность в нём, подчеркни его сильные стороны. "
        ...
        f"{user_text}\n\n"
        "Теперь придумай одно короткое поддерживающее сообщение, обращаясь к Максиму. Только текст сообщения, без пояснений."
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == "private":
        await update.message.reply_text(
            ...
            "В чате я шучу над Максимом и одновременно поддерживаю его, "
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if SUPPORT_USER_ID is not None and user_id == SUPPORT_USER_ID:
        ...
        fallback = "Максим, ты реально круто справляешься, даже если сам этого не замечаешь."
        reply_text = generate_ai_text(prompt, fallback)
